"""LowBody flat velocity environment configuration."""
...
from src.assets.robots.jljbot.constrant import (
  LOWBODY_ACTION_SCALE,
  LOWBODY_FOOT_COLLISION_NAMES,
  get_lowbody_robot_cfg,
)
...
_FOOT_SUBTREE_PATTERN = r"^(left_ankle_roll_link|right_ankle_roll_link)$"

LOWBODY_ENABLE_MASS_RANDOMIZATION = True
LOWBODY_MASS_SCALE_RANGE = (0.9, 1.2)

LOWBODY_ENABLE_CONTROL_DELAY_RANDOMIZATION = True
LOWBODY_CONTROL_DELAY_LAG_RANGE = (0, 3)

LOWBODY_ENABLE_FOOT_HARDNESS_RANDOMIZATION = True
LOWBODY_FOOT_SOLREF_TIMECONST_RANGE = (0.004, 0.02)

LOWBODY_ENABLE_JOINT_FRICTION_RANDOMIZATION = True
LOWBODY_JOINT_FRICTION_RANGE = (0.0, 0.6)
...
def _validate_randomization_ranges() -> None:
  delay_min_lag, delay_max_lag = LOWBODY_CONTROL_DELAY_LAG_RANGE
  ...
  hardness_min, hardness_max = LOWBODY_FOOT_SOLREF_TIMECONST_RANGE
  ...
  friction_min, friction_max = LOWBODY_JOINT_FRICTION_RANGE
...
def lowbody_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create LowBody flat-terrain velocity configuration."""
    ...
    "foot_friction": EventTermCfg(
      ...
      params={
        "asset_cfg": SceneEntityCfg(
          "robot", geom_names=LOWBODY_FOOT_COLLISION_NAMES
        ),
        "operation": "abs",
        "ranges": (0.1, 1.6),
        "shared_random": True,
  ...
  if LOWBODY_ENABLE_MASS_RANDOMIZATION and not play:
    events["body_mass"] = EventTermCfg(
      ...
      params={
        ...
        "alpha_range": _body_mass_scale_range_to_alpha_range(
          LOWBODY_MASS_SCALE_RANGE
        ),
      },
    )

  if LOWBODY_ENABLE_FOOT_HARDNESS_RANDOMIZATION and not play:
    events["foot_hardness"] = EventTermCfg(
      ...
      params={
        ...
        "ranges": LOWBODY_FOOT_SOLREF_TIMECONST_RANGE,
        "shared_random": True,
      },
    )

  if LOWBODY_ENABLE_JOINT_FRICTION_RANDOMIZATION and not play:
    events["joint_friction"] = EventTermCfg(
      ...
      params={
        "asset_cfg": SceneEntityCfg("robot", joint_names=(".*",)),
        "operation": "abs",
        "ranges": LOWBODY_JOINT_FRICTION_RANGE,
        "shared_random": False,
      },
    )
  ...
  cfg = ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      entities={
        "robot": get_lowbody_robot_cfg(
          enable_control_delay=(
            LOWBODY_ENABLE_CONTROL_DELAY_RANDOMIZATION and not play
          ),
          control_delay_min_lag=LOWBODY_CONTROL_DELAY_LAG_RANGE[0],
          control_delay_max_lag=LOWBODY_CONTROL_DELAY_LAG_RANGE[1],
        )
      },
      num_envs=1,
      extent=2.0,
